...
void add_lists(Node *list1, Node *list2)
{
  if ((list1 == NULL) || (list2 == NULL)) //ends if either of the lists are empty
    return; 
  list1->value = (list1->value) + (list2->value); //adds the list values together at the current head
  if ((list1->next == NULL) || (list2->next == NULL))//checks to ensure the next entries exists
    return; 
  add_lists(list1->next, list2->next); //recursively calls at the next value
}
...
Node *duplicate_list(Node *head)
{
  if (head != NULL)
  {
    ...
    clone->value = head->value; //sets the value of the new list equal to the head
    clone->next = duplicate_list(head->next); //recursively calls the next value and sets the new list equal to it
    return clone;
  }
  else
  {
...
Node *efficient_delete_match(Node *head, int delete_value, int *num_deleted)
{
  ...
  *num_deleted = 0;
  ...
  while (head != NULL && delete_value == head->value) //ensures that the head is not null and checks if the delete value matches
  {
    head = head->next; //moves the head value
    (*num_deleted)++; //increments the numbers deleted

  }
  Node *new_head = head;
  while (new_head->next != NULL) { //while the next value in the list exists

    if (new_head->next->value == delete_value){ //chjecks the next value in the list
      Node *deleted_item = new_head->next;
      new_head->next = new_head->next->next;
      free(deleted_item);//frees the value being deleted
      (*num_deleted)++; 

    }else{

      new_head = new_head->next; //goes to the next value if it does not match delete_value
